...
const Dashboard = () => {
  ...
  const { token, partnerName, logout } = useAuth();
  ...
  const dashboardItems = [
    { title: 'Generate Invoice', description: 'Create new invoices for your customers', icon: FileText, color: 'bg-green-500', path: '/generate-invoice' },
    { title: 'Report Generation', description: 'Generate purchase and sales reports', icon: FileBarChart, color: 'bg-cyan-500', path: '/report-generation' },
    { title: 'Record Payment', description: 'Record customer payments', icon: CreditCard, color: 'bg-teal-500', path: '/record-payment' },
    { title: 'Add Purchase Entry', description: 'Add purchase order details', icon: ShoppingCart, color: 'bg-yellow-500', path: '/add-purchase-entry' },
    { title: 'Record Purchase Payment', description: 'Record payments for purchase orders', icon: Receipt, color: 'bg-pink-500', path: '/record-purchase-payment' },
    { title: 'Cancel Invoice', description: 'Cancel or void existing invoices', icon: XCircle, color: 'bg-red-500', path: '/cancel-invoice' },
    { title: 'Add Customer', description: 'Add new customer information', icon: Users, color: 'bg-blue-500', path: '/add-customer' },
    { title: 'Add Product', description: 'Add new products to your catalog', icon: Package, color: 'bg-purple-500', path: '/add-product' },
    { title: 'Update Customer', description: 'Modify existing customer details', icon: Users, color: 'bg-orange-500', path: '/update-customer' },
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Statistics + Sales Balance List side-by-side */}
        <div className="flex flex-col md:flex-row gap-6 mt-6">
          ...
          <Card
            className="flex items-center gap-4 p-5 bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow cursor-pointer"
            onClick={() => navigate('/sales-balance-list')}
          >
            <div className="bg-green-500 p-3 rounded-lg text-white flex items-center justify-center">
              <TrendingUp className="w-6 h-6" />
            </div>
            <div>
              <h3 className="text-base font-semibold text-gray-900">Sales Balance List</h3>
              <p className="text-sm text-gray-500">View pending sales balances</p>
            </div>
          </Card>
        </div>
      </div>
